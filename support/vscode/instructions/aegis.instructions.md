...
Require delegated workers to produce a fresh `FINAL HANDOFF`; use `aegis last-summary` or cell handoff records before reading full transcripts. If `cell session sync` marks a session `needs-restart-or-nudge`, treat it as recoverable coordinator action rather than a blocker: restart or nudge the backend session and keep any real blocker records separate.
