...
public class CommandRulesTests
{
    ...
    {
        var directory = LocateRepoRoot(Directory.GetCurrentDirectory());
        var coreProject = Path.Combine(directory, "src", "Aegis.Core", "Aegis.Core.csproj");
        var cliProject = Path.Combine(directory, "src", "Aegis", "Aegis.csproj");
        var cliProjectText = File.ReadAllText(cliProject);

        Assert.True(File.Exists(coreProject), "Aegis.Core must exist as a reusable library project.");
        Assert.Contains("Aegis.Core.csproj", cliProjectText);
        Assert.True(File.Exists(Path.Combine(directory, "src", "Aegis.Core", "Core", "BackendContracts.cs")));
        Assert.True(File.Exists(Path.Combine(directory, "src", "Aegis.Core", "Infrastructure", "SessionRegistryService.cs")));
        Assert.False(Directory.Exists(Path.Combine(directory, "src", "Aegis", "Core")));
        Assert.False(Directory.Exists(Path.Combine(directory, "src", "Aegis", "Infrastructure")));
    }
    ...
    {
        var directory = LocateRepoRoot(Directory.GetCurrentDirectory());
        var backendProject = Path.Combine(directory, "src", "Aegis.Backends", "Aegis.Backends.csproj");
        var cliProject = Path.Combine(directory, "src", "Aegis", "Aegis.csproj");
        var cliProjectText = File.ReadAllText(cliProject);

        Assert.True(File.Exists(backendProject), "Aegis.Backends must exist as a reusable backend adapter project.");
        Assert.Contains("Aegis.Backends.csproj", cliProjectText);
        Assert.True(File.Exists(Path.Combine(directory, "src", "Aegis.Backends", "Backends", "CodexBackend.cs")));
        Assert.True(File.Exists(Path.Combine(directory, "src", "Aegis.Backends", "Backends", "OpenCodeBackend.cs")));
        Assert.True(File.Exists(Path.Combine(directory, "src", "Aegis.Backends", "Backends", "PiBackend.cs")));
        Assert.False(Directory.Exists(Path.Combine(directory, "src", "Aegis", "Backends")));
    }
    ...
    {
        var directory = LocateRepoRoot(Directory.GetCurrentDirectory());
        var cliProjectPath = Path.Combine(directory, "src", "Aegis", "Aegis.csproj");
        var legacyProjectPath = Path.Combine(directory, "src", "Aegis", "OpencodeAegis.csproj");
        var cliProject = File.ReadAllText(cliProjectPath);
        var solution = File.ReadAllText(Path.Combine(directory, "aegis.sln"));

        Assert.True(File.Exists(cliProjectPath), "CLI app project should use the backend-agnostic Aegis name.");
        ...
        Assert.Contains("aegis.cmd", cliProject);
        Assert.Contains("opencode-aegis.cmd", cliProject);
        Assert.Contains("src\\Aegis\\Aegis.csproj", solution);
        Assert.DoesNotContain("OpencodeAegis", solution);
    }
    ...
    {
        var directory = LocateRepoRoot(Directory.GetCurrentDirectory());
        var coreProject = File.ReadAllText(Path.Combine(directory, "src", "Aegis.Core", "Aegis.Core.csproj"));
        var backendProject = File.ReadAllText(Path.Combine(directory, "src", "Aegis.Backends", "Aegis.Backends.csproj"));

        Assert.Contains("<PackageId>Aegis.Core</PackageId>", coreProject);
        Assert.Contains("<PackageId>Aegis.Backends</PackageId>", backendProject);
