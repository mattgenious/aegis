...
- `Aegis.Core`: shared contracts, session registry infrastructure, state normalization, and prompt rendering.
- `Aegis.Backends`: OpenCode, Codex, Pi, and GitHub Copilot CLI backend adapters plus backend command orchestration.

The CLI executable is `aegis` and references the same libraries. `aegis` and `opencode-aegis` are migration aliases for installed or published command shims, not separate application identities.
...
```bash
dotnet pack src/Aegis.Core/Aegis.Core.csproj -c Release -o artifacts/packages
