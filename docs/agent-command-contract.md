# Aegis Agent Command Contract

This document records the command forms and output expectations that coordinator agents, agent hosts, maintainers, and smoke tests rely on.

It is not a human onboarding tutorial. Aegis is a CLI-shaped contract because command invocations are easy for agents to emit, supervise, and recover across local backend tools.
...
Coordinator agents should detect backends before choosing a delegated backend:
...
Library/package consumption notes for in-process agent hosts are in [package-consumption.md](./package-consumption.md).
...
The standalone installer publishes versioned `aegis.exe` builds under `$HOME\.local\aegis\versions`, installs a primary PATH shim at `$HOME\.local\aegis\bin\aegis.cmd`, updates the active marker, and prunes old versions by default. Pass `-DryRun` to preview and `-InstallRoot <path>` to target a different install directory.
...
The VS Code support installer copies only the Aegis templates from `support/vscode/` into `$HOME/.copilot/agents`, `$HOME/.copilot/instructions`, and `$HOME/.copilot/prompts`. Pass `-DryRun` on PowerShell or `--dry-run` on POSIX shells to preview. Pass `-TargetRoot <path>` / `--target-root <path>` or `-ProfileRoot <path>` / `--profile-root <path>` to install into a test directory or a non-default Copilot profile root. Pass `-WorkspaceRoot <path>` / `--workspace-root <path>` to also install workspace-scoped copies under `.github/`.

VS Code support is terminal/tool based: VS Code agents can launch Aegis and can spawn or supervise external backend sessions through OpenCode, Codex, GitHub Copilot CLI, or other supported Aegis backends. Aegis does not drive VS Code's native Copilot Chat UI as a backend session host.
...
After install, bootstrap environments should ensure PATH resolves the higher-priority shim instead of any older locked executable.
...
Command form for starting or verifying a local unauthenticated OpenCode server:
...
These command forms document how coordinator agents launch a worker session and extract the final handoff summary:
...
Coordinator agents use `--async` when they need to return immediately and fetch the summary later:
...
Coordinator agents use `spawn` to launch multiple implementation sessions without hand-rolling OpenCode API loops:
...
Coordinator agents use `cell` when they need durable state for a recursive coordination graph: cells, child cells, workstreams, roles, clones, sessions, evidence, final handoffs, blockers, and integration notes.
...
Coordinator agents create a cell, attach clone-backed workstreams, fork child cells when a worker needs to split work further, fan out worker sessions, and render an optional observer view:
...
Coordinator agents keep the cell current as workers report back:
...
aegis cell session verify --session codex-... --kind parent-review --result pass --summary "Diff and tests checked by coordinator agent."
```
...
When a backend session has stopped without a fresh final handoff, `cell session sync` records it as `needs-restart-or-nudge` so coordinator agents can restart or nudge it without mistaking it for a blocker.
...
- Coordinator agents should use `--model github-copilot/gpt-5.4-mini --variant low` for fast delegated work when no repo-specific profile overrides it.
- Coordinator agents should reserve `--model github-copilot/gpt-5.5 --variant high` or `--variant xhigh` for small delegated tasks that justify higher reasoning cost.
