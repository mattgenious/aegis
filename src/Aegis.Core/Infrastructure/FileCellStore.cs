...
public sealed class DefaultCellPathProvider : ICellPathProvider
{
    ...
    private static string ResolvePath()
    {
        ...
        if (!string.IsNullOrWhiteSpace(appData))
        {
            ...
            var legacyPath = Path.Combine(appData, "aegis", "cell");
        ...
        var tempLegacyPath = Path.Combine(Path.GetTempPath(), "aegis", "cell");
        return !Directory.Exists(tempAegisPath) && Directory.Exists(tempLegacyPath) ? tempLegacyPath : tempAegisPath;
    }
}

public sealed class FileCellStore : ICellStore
{
    private const int MaxFileAttempts = 25;

    private readonly ICellPathProvider _pathProvider;
    private readonly JsonSerializerOptions _options = new(JsonSerializerDefaults.Web)
    {
        WriteIndented = true
    };

    public FileCellStore(ICellPathProvider? pathProvider = null)
    {
        _pathProvider = pathProvider ?? new DefaultCellPathProvider();
    }

    public string DirectoryPath => _pathProvider.DirectoryPath;

    public Task SaveMissionAsync(CellMissionRecord mission, CancellationToken cancellationToken = default) =>
        SaveAsync(PathFor("missions", mission.Id), mission, cancellationToken);

    public Task<CellMissionRecord?> GetMissionAsync(string missionId, CancellationToken cancellationToken = default) =>
        LoadAsync<CellMissionRecord>(PathFor("missions", missionId), cancellationToken);

    public Task<IReadOnlyList<CellMissionRecord>> GetMissionsAsync(CancellationToken cancellationToken = default) =>
        LoadAllAsync<CellMissionRecord>("missions", cancellationToken);

    public Task<CellMissionRecord> UpdateMissionAsync(
        string missionId,
        Func<CellMissionRecord, CellMissionRecord> update,
        CancellationToken cancellationToken = default) =>
        UpdateAsync(PathFor("missions", missionId), update, cancellationToken);

    public Task SaveWorkstreamAsync(CellWorkstreamRecord workstream, CancellationToken cancellationToken = default) =>
        SaveAsync(PathFor("workstreams", workstream.Id), workstream, cancellationToken);

    public Task<CellWorkstreamRecord?> GetWorkstreamAsync(string workstreamId, CancellationToken cancellationToken = default) =>
        LoadAsync<CellWorkstreamRecord>(PathFor("workstreams", workstreamId), cancellationToken);

    public Task<IReadOnlyList<CellWorkstreamRecord>> GetWorkstreamsAsync(CancellationToken cancellationToken = default) =>
        LoadAllAsync<CellWorkstreamRecord>("workstreams", cancellationToken);

    public async Task<IReadOnlyList<CellWorkstreamRecord>> GetWorkstreamsAsync(
        string missionId,
        CancellationToken cancellationToken = default)
    {
    ...
    public Task<CellWorkstreamRecord> UpdateWorkstreamAsync(
        string workstreamId,
    ...
    public Task SaveAgentSessionAsync(CellAgentSessionRecord session, CancellationToken cancellationToken = default) =>
        SaveAsync(PathFor("sessions", session.Id), session, cancellationToken);

    public Task<CellAgentSessionRecord?> GetAgentSessionAsync(string sessionId, CancellationToken cancellationToken = default) =>
        LoadAsync<CellAgentSessionRecord>(PathFor("sessions", sessionId), cancellationToken);

    public Task<IReadOnlyList<CellAgentSessionRecord>> GetAgentSessionsAsync(CancellationToken cancellationToken = default) =>
        LoadAllAsync<CellAgentSessionRecord>("sessions", cancellationToken);

    public async Task<IReadOnlyList<CellAgentSessionRecord>> GetAgentSessionsAsync(
        string missionId,
        CancellationToken cancellationToken = default)
    {
    ...
    public Task<CellAgentSessionRecord> UpdateAgentSessionAsync(
        string sessionId,
